...
void ArmLowLevel::updateCommand(LowlevelCmd* cmd){
    ...
    cmd->motorCmd[12].q = armCommand[0].qDes[2]; //right elbow 
    cmd->motorCmd[12].Kp = 10;
    cmd->motorCmd[12].Kd = 1;

    cmd->motorCmd[13].q = armCommand[1].qDes[0]; //left twist
    cmd->motorCmd[13].Kp = 10;
    cmd->motorCmd[13].Kd = 1;
    cmd->motorCmd[14].q = armCommand[1].qDes[1]; //left shoulder
    cmd->motorCmd[14].Kp = 10;
    cmd->motorCmd[14].Kd = 1;
    cmd->motorCmd[15].q = armCommand[1].qDes[2]; //left elbow
